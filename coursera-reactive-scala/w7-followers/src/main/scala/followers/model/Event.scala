...
/** This object provides a representation for the various incoming events.
  ...
  * be notified Broadcast: All connected user clients should be notified Private
  * Message: Only the To User Id should be notified Status Update: All current
  * followers of the From User ID should be notified
  */
sealed trait Event:
...
object Event:
  ...
  final case class Follow(
    ...
    def render: ByteString = ByteString(
      s"$sequenceNr|F|$fromUserId|$toUserId\n"
    )
  ...
  final case class Unfollow(
    ...
    def render: ByteString = ByteString(
      s"$sequenceNr|U|$fromUserId|$toUserId\n"
    )
  ...
  final case class PrivateMsg(
    ...
    def render: ByteString = ByteString(
      s"$sequenceNr|P|$fromUserId|$toUserId\n"
    )

  /** Status Update: All current followers of the From User ID should be
    * notified
    */
