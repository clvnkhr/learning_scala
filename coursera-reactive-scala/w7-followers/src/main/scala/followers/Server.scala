...
import followers.model.Event.Broadcast
import followers.model.Event.PrivateMsg
import followers.model.Event.StatusUpdate
...
object Server extends ServerModuleInterface:
  ...
  /** A flow that consumes chunks of bytes and produces `String` messages.
    *
    * Each incoming chunk of bytes doesn’t necessarily contain ''exactly one''
    * message payload (it can contain fragments of payloads only). You have to
    * process these chunks to produce ''frames'' containing exactly one message
    * payload.
    *
  ...
  /** A flow that consumes chunks of bytes and produces [[Event]] messages.
    ...
    * message payload (it can contain fragments of payloads only). You have to
    * process these chunks to produce frames containing exactly one message
    * payload before you can parse such messages with [[Event.parse]].
    *
  ...
  /** Implement a Sink that will look for the first [[Identity]] (we expect
    ...
    *
    * Note that you may need the Sink's materialized value; you may want to
    * compare the signatures of `Flow.to` and `Flow.toMat` (and have a look at
    * `Keep.right`).
    */
  ...
  /** A flow that consumes unordered messages and produces messages ordered by
    * `sequenceNr`.
    *
    * User clients expect to be notified of events in the correct order,
    * regardless of the order in which the event source sent them.
    *
    * You will have to buffer messages with a higher sequence number than the
    * next one expected to be produced. The first message to produce has a
    * sequence number of 1.
    *
    * You may want to use `statefulMapConcat` in order to keep the state needed
  ...
  /** @return
    ...
    *   the current state of `Followers`. See [[Event]] for more information of
    *   when users should be notified about them.
    * @param userId
    *   Id of the user
    * @param eventAndFollowers
    *   Event and current state of followers
    */
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers match
      case (Follow(_, _, toUserId), _)     => userId == toUserId
...
/** Creates a hub accepting several client connections and a single event
  * connection.
  *
  * @param executionContext
  *   Execution context for `Future` values transformations
  * @param materializer
  *   Stream materializer
  */
class Server(using ExecutionContext, Materializer)
  ...
  /** The hub is instantiated here. It allows new user clients to join
    * afterwards and receive notifications from their event feed (see the
    * `clientFlow()` member below). The hub also allows new events to be pushed
    * to it (see the `eventsFlow` member below).
    *
  ...
  /** The "final form" of the event flow.
    *
    * It consumes byte strings which are the events, and feeds them to the hub
    * inbound.
    *
    * The server does not need to write any data back to the event source (use
    * `Source.maybe` to represent something that does not write data, yet at the
    * same time does NOT complete the stream, otherwise the connection could be
    * closed).
    *
  ...
  /** @return
    ...
    *
    * Follow: Only the To User Id should be notified Unfollow: No clients should
    * be notified Broadcast: All connected user clients should be notified
    * Private Message: Only the To User Id should be notified Status Update: All
  ...
  /** The "final form" of the client flow.
    *
    * Clients will connect to this server and send their id as an Identity
    * message (e.g. "21323\n").
    *
    * The server should establish a link from the event source towards the
    * clients, in such way that they receive only the events that they are
    * interested about.
    *
    * The incoming side of this flow needs to extract the client id to then
    * properly construct the outgoing Source, as it will need this identifier to
    * notify the server which data it is interested about.
    *
    * Hints:
    *   - since the clientId will be emitted as a materialized value of
    *     `identityParserSink`, you may need to use mapMaterializedValue to side
    *     effect it into a shared Promise/Future that the Source side can
    *     utilise to construct such Source "from that client id future".
    *   - Remember to use `via()` to connect a `Flow`, and `to()` to connect a
    *     `Sink`.
    */
