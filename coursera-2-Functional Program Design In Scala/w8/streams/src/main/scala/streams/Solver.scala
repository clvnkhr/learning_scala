...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val frontier =
      for 
        (b1, moves1) <- initial
        if !done(b1)
        (b2, moves2) <- newNeighborsOnly(
          neighborsWithHistory(b1, moves1), explored
        )
      yield (b2, moves2)

    if frontier.isEmpty then initial
    else    
      initial #::: from(frontier, explored ++ initial.map((b,_)=>b))
  ...
  lazy val solution: List[Move] = 
    val allSolutions = pathsToGoal
    if allSolutions.isEmpty then Nil else allSolutions.head(1).reverse